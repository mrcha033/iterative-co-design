...
class TestLatencyProfiler:
    ...
    def test_measure_latency_cpu(self):
        ...
        dummy_input = {"input_ids": torch.randn(2, 10, 32)}  # Proper shape: (batch, seq, features)
    ...
    def test_measure_cache_hits_subprocess_success(self, mock_subprocess):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('torch.cuda.is_available', return_value=True), \
                 patch('shutil.which', return_value='/usr/bin/ncu'), \
                 patch.object(profiler, '_parse_ncu_output', return_value=mock_metrics), \
    ...
    def test_parse_ncu_output_success(self):
        ...
        profiler = LatencyProfiler(ncu_metrics=["l2_tex_hit_rate.pct", "sm__cycles_elapsed.avg"])
