...
Examples:
  ...
  python scripts/setup.py --test            # Run tests after installation
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run tests after installation"
    )
    parser.add_argument(
        "--skip-pytorch",
        action="store_true",
    ...
    if not run_command(
        ...
        print(
            "Failed to install from requirements.txt. Trying with --no-deps to diagnose..."
        )
    ...
    if success:
        ...
        verification_commands = [
            ...
            (
                "python -c \"from utils.config import load_yaml_config; print('Config import successful')\"",
                "Config module",
            ),
            (
                "python -c \"from co_design.modularity import calculate_modularity; print('Modularity module successful')\"",
                "Modularity module",
            ),
            (
                "python -c \"from models.wrapper import ModelWrapper; print('Model wrapper successful')\"",
                "Model wrapper",
            ),
        ]
    ...
    print("\nNext steps:")
    print(
        "1. Run experiments: python scripts/run_experiment.py model=mamba_3b dataset=wikitext103 method=dense"
    )
