...
def setup_model_and_data(model_name="state-spaces/mamba-2.8b-slimpj", 
...
def generate_figure1(quick_mode=False):
    ...
    try:
        ...
        plt.savefig(output_dir / "figure1_mamba_latency_scan_vs_perm.pdf", dpi=300, bbox_inches='tight')
...
def generate_figure2():
    ...
    try:
        ...
        if not quant_dir.exists():
            print("⚠️ No quantization results found. Running experiments...")
        ...
        plt.title('Figure 2: The Value of Iteration in Co-Design with Quantization\n'
                  'Iterative approach outperforms linear pipelines', fontsize=16, pad=20)
        ...
        output_dir = Path("figures")
        output_dir.mkdir(exist_ok=True)
        plt.savefig(output_dir / "figure2_quantization_results.pdf", dpi=300, bbox_inches='tight')
        plt.savefig(output_dir / "figure2_quantization_results.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def generate_figure3():
    ...
    try:
        ...
        line1 = ax1.plot(iterations, iter_metrics['latency'], 'b-o', 
        ...
        line2 = ax2.plot(iterations, iter_metrics['modularity'], 'o-', 
                        color='orange', linewidth=3, markersize=8, label='Modularity')
        line3 = ax2.plot(iterations, iter_metrics['l2_cache_hit_rate'], 's-', 
                        color='green', linewidth=3, markersize=8, label='L2 Cache Hit Rate')
        ...
        output_dir = Path("figures")
        output_dir.mkdir(exist_ok=True)
        plt.savefig(output_dir / "figure3_metrics_vs_iteration.pdf", dpi=300, bbox_inches='tight')
        plt.savefig(output_dir / "figure3_metrics_vs_iteration.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def generate_figure4():
    ...
    try:
        ...
        if not records:
            print("❌ No valid experimental data found with both perplexity and latency")
            return False
        ...
        for _, row in df.iterrows():
            method = row['method']
            style = method_styles.get(method, {'color': 'gray', 'marker': 'o', 'size': 100})
            
            label = f"{method} (Ours)" if method == 'iterative' else method
            
            plt.scatter(row['perplexity'], row['latency_ms'], 
        ...
        plt.title('Figure 4: Pushing the Pareto Frontier\n'
                  'Iterative Co-Design establishes new state-of-the-art', fontsize=16, pad=20)
        ...
        output_dir = Path("figures")
        output_dir.mkdir(exist_ok=True)
        plt.savefig(output_dir / "figure4_pareto_frontier.pdf", dpi=300, bbox_inches='tight')
        plt.savefig(output_dir / "figure4_pareto_frontier.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    if figures_dir.exists():
        figure_files = sorted(list(figures_dir.glob("*.pdf")) + list(figures_dir.glob("*.png")))
        if figure_files:
